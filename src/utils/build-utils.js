...
export const buildIosRelease = (archiveCommand, isStaging) => {
    ...
    let command = `${yarnCommand} && ${patchCommand} && cd ${iosProjectDirectory} && pod install && ${archiveCommand} -allowProvisioningUpdates && ${exportIPACommand} && cd ${autoFlipDirectory} && node index-ios.js`;
    if (isStaging) {
...
export const buildIosStagingReleaseFirebase = () => {
    const archiveCommand = 'xcodebuild -workspace FlipApp.xcworkspace -scheme FlipAppStaging -sdk iphoneos -configuration Staging archive -archivePath flip.xcarchive';

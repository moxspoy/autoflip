...
const { ANDROID_VERSION_CODE, ANDROID_VERSION_NAME, IOS_VERSION_NAME, IOS_VERSION_CODE } = process.env;

export const uploadSourceMapsAndroidToSentry = () => {
    console.log('uploading into sentry (android)...');
    const command =
        `cd ${flipMobileDirectory} && node_modules/@sentry/cli/bin/sentry-cli releases --org flipid --project flip-mobile files "id.flip@${ANDROID_VERSION_NAME}@${ANDROID_VERSION_CODE}+codepush:1" upload-sourcemaps --dist ${ANDROID_VERSION_CODE}.1 --strip-prefix . --bundle android/app/build/generated/assets/react/production/release/index.android.bundle --bundle-sourcemap android/app/build/generated/sourcemaps/react/production/release/index.android.bundle.map`;
...
export const uploadSourceMapsIosToSentry = () => {
    ...
    const uploadCommand =
        `cd ${flipMobileDirectory} && node_modules/@sentry/cli/bin/sentry-cli releases --org flipid --project flip-mobile files "id.flip.release@${IOS_VERSION_NAME}@${IOS_VERSION_CODE}+codepush:1" upload-sourcemaps --dist ${IOS_VERSION_NAME}.1 --strip-prefix . --bundle main.jsbundle --bundle-sourcemap main.jsbundle.map`;
